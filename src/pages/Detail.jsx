...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      disptach(removeSelectedMovieOrShow());
    };
  }, [disptach, imdbID]);
    ...
    <div className="grid grid-cols-2 h-[calc(100vh-9.7rem)]">
      {Object.keys(info).length === 0 ? (
        <div className="flex justify-center items-center">
          <h1 className="text-4xl font-extrabold">Cargando...</h1>
        </div>
      ) : (
        <>
          <div className="flex flex-col justify-center">
            ...
            <h3 className="text-lg text-left py-3">{info.Plot}</h3>
            <div className="flex flex-col gap-3">
              <p>
                <span className="font-extrabold">Director:</span>{" "}
                {info.Director}
              </p>
              <p>
